...
#include "hipe_slave_gc.h"
#endif

static Eterm *master_fullsweep_nstack(Process *p, Eterm *n_htop)
{
...
/*
 * This is really icky, but we can't screw up the performance of
 * fullsweep_nstack when the slave is disabled
 */
#ifdef ERTS_SLAVE_EMU_ENABLED
static Eterm *slave_fullsweep_nstack(Process *p, Eterm *n_htop)
{
    /* known nstack walk state */
    Eterm *nsp;
    Eterm *nsp_end;
    ...
    unsigned int mask;
    /* arch-specific nstack walk state */
    struct slave_nstack_walk_state walk_state;
    ...
    if (!slave_nstack_walk_init_check(p))
	return n_htop;

    ASSERT(p->hipe.nsp && p->hipe.nstend);
    nsp = slave_nstack_walk_nsp_begin(p);
    nsp_end = p->hipe.nstgraylim;
    if (nsp_end)
	slave_nstack_walk_kill_trap(p, nsp_end);
    nsp_end = slave_nstack_walk_nsp_end(p);

    sdesc = slave_nstack_walk_init_sdesc(p, &walk_state);
    ...
    for (;;) {
	if (slave_nstack_walk_nsp_reached_end(nsp, nsp_end)) {
	    if (nsp == nsp_end) {
		if (nsp) {
		    /* see the HIGH_WATER update in fullsweep_heap() */
		    p->hipe.nstblacklim = nsp; /* nsp == nsp_end */
		    slave_nstack_walk_update_trap(p, walk_state.sdesc0);
		}
		return n_htop;
	    }
	    fprintf(stderr, "%s: passed end of stack\r\n", __FUNCTION__);
	...
	sdesc_size = slave_nstack_walk_frame_size(sdesc);
	i = 0;
	mask = sdesc->livebits[0];
	for (;;) {
	    if (mask & 1) {
		Eterm *nsp_i = slave_nstack_walk_frame_index(nsp, i);
		Eterm gval = *nsp_i;
		if (is_boxed(gval)) {
		    Eterm *ptr = boxed_val(gval);
		    Eterm val = *ptr;
	...
	ra = slave_nstack_walk_frame_ra(nsp, sdesc);
	sdesc = hipe_slave_find_sdesc(ra);
	nsp = slave_nstack_walk_next_frame(nsp, sdesc_size);
    }
    abort();
}
#endif
...
Eterm *fullsweep_nstack(Process *p, Eterm *n_htop)
{
	...
	return slave_fullsweep_nstack(p, n_htop);
#endif
    return master_fullsweep_nstack(p, n_htop);
}

static void master_gensweep_nstack(Process *p, Eterm **ptr_old_htop, Eterm **ptr_n_htop)
{
    /* known nstack walk state */
    Eterm *nsp;
    Eterm *nsp_end;
...
/* Ditto about ickyness */
#ifdef ERTS_SLAVE_EMU_ENABLED
static void slave_gensweep_nstack(Process *p, Eterm **ptr_old_htop, Eterm **ptr_n_htop)
{
    /* known nstack walk state */
    Eterm *nsp;
    Eterm *nsp_end;
    ...
    if (nsp_end) {
	/* if gray limit passed black limit, reset black limit */
	if (slave_nstack_walk_gray_passed_black(nsp_end, p->hipe.nstblacklim))
	    p->hipe.nstblacklim = nsp_end;
	slave_nstack_walk_kill_trap(p, nsp_end);
	nsp_end = p->hipe.nstblacklim;
    } else
...
#ifdef ERTS_SLAVE_EMU_ENABLED
    if (IS_SLAVE_PROCESS(p))
	return slave_gensweep_nstack(p, ptr_old_htop, ptr_n_htop);
#endif
    return master_gensweep_nstack(p, ptr_old_htop, ptr_n_htop);
}
