...
/* For the -s flag (slave literals for the master). */
static const char *macro_prefix = "";
...
{
    fprintf(fp, "#define %s%s %d\n", macro_prefix, literal->name, literal->value);
}
...
{
    fprintf(fp, "-define(%s%s, %s).\n", macro_prefix, atom_literal->name, atom_literal->value);
}
...
{
    if (param->is_defined)
	fprintf(fp, "#define %s%s %d\n", macro_prefix, param->name, param->value);
}
...
{
    fprintf(fp, "/* File: hipe_%sliterals.h, generated by %s */\n", macro_prefix, this_exe);
    fprintf(fp, "#ifndef __HIPE_%sLITERALS_H__\n", macro_prefix);
    fprintf(fp, "#define __HIPE_%sLITERALS_H__\n\n", macro_prefix);
...
{
    fprintf(fp, "%%%% File: hipe_%sliterals.hrl, generated by %s", macro_prefix, this_exe);
    fprintf(fp, "\n\n");
...
error:
    fprintf(stderr, "usage: %s [-x] [-s] [-c | -e] > output-file\n"
	    "\t-c\tC header file\n"
	    "\t-e\tErlang header file\n"
	    "\t-x\tCross compile. No dependencies to compiling emulator\n"
	    "\t-s\tSlave prefix. All macros prefixed by S_.\n",
	    argv[0]);
    return 1;
}
