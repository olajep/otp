...
export_alloc(struct export_entry* tmpl_e)
{
	...
	obj->code[3] = (BeamInstr) SlaveOp(op_call_error_handler);
	obj->code[4] = 0;
...
slave_find_export_entry(Eterm m, Eterm f, unsigned int a,ErtsCodeIndex code_ix);
...
/* Export* */
/* slave_find_function(Eterm m, Eterm f, unsigned int a, ErtsCodeIndex code_ix) */
/* { */
...
/* 	 ee->ep->code[3] != (BeamInstr) SlaveOp(op_i_generic_breakpoint))) { */
/* 	return NULL; */
/*     } */
/*     return ee->ep; */
/* } */
...
Export*
slave_export_get_or_make_stub(Eterm mod, Eterm func, unsigned int arity)
{
