...
alloc(Defun, CFG, SpillLimit, SpillIndex, Options, RegAllocMod, TargetMod) ->
  ...
  case RegAllocMod:regalloc(CFG, SpillIndex, SpillLimit,TargetMod, Options) of
    {Coloring, _NewSpillIndex} -> Liveness0 = undefined;
    {Coloring, _NewSpillIndex, Liveness0} -> ok
  end,
  ...
  case DidSpill of
    false -> %% No new temps, we are done.
      ...
      Liveness = case Liveness0 of
		   undefined -> TargetMod:analyze(CFG);
		   _ -> Liveness0
		 end,
