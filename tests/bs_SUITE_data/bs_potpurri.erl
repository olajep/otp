...
test() ->
  ?line ok = integer(),
  ?line ok = signed_integer(),
  ?line ok = dynamic(),
  ?line ok = more_dynamic(),
  ?line ok = mml(),
  ok.
...
dynamic() ->
  ?line ok = dynamic(mkbin([255]), 8),
  ?line ok = dynamic(mkbin([255,255]), 16),
  ?line ok = dynamic(mkbin([255,255,255]), 24),
  ?line ok = dynamic(mkbin([255,255,255,255]), 32),
  ok.
...
more_dynamic() ->
  ...
  ?line ok = more_dynamic1(Unsigned, funny_binary(42)),
  ...
  ?line ok = more_dynamic1(Signed, funny_binary(43)),
  ...
  ?line ok = more_dynamic1(SignLittle, funny_binary(45)),
