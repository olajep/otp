...
all() -> 
    [bs_add, otp_7422, zero_width, bad_append,
     ...
     copy_writable_binary, kostis, dynamic
    ].
...
dynamic_1(Dynamic) ->
    <<Lpad:32,_/binary>> = erlang:md5([0]),
    <<Rpad:32,_/binary>> = erlang:md5([1]),
    <<Int:32,_/binary>> = erlang:md5([2]),
    561 = dynamic_2(0, {Int,Lpad,Rpad,Dynamic}, 0).

dynamic_2(33, _, Count) -> Count;
...
dynamic_big(Bef, N, Int, Lpad, Rpad) ->
    ...
    Bin = id(<<Lpad:Bef,NumBin/bitstring,Rpad:(32-Bef-N)>>),
    Bin = <<Lpad:Bef,Int:N,Rpad:(32-Bef-N)>>,

    %% Further verify the result by matching.
    LpadMasked = Lpad band ((1 bsl Bef) - 1),
    RpadMasked = Rpad band ((1 bsl (32-Bef-N)) - 1),
    Rbits = (32-Bef-N),
...
dynamic_little(Bef, N, Int, Lpad, Rpad) ->
    ...
    %% Construct the binary in two different ways.
    Bin = id(<<Lpad:Bef/little,NumBin/bitstring,Rpad:(32-Bef-N)/little>>),
    Bin = <<Lpad:Bef/little,Int:N/little,Rpad:(32-Bef-N)/little>>,
