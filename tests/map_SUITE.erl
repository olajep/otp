...
all() -> [
	...
	t_maps_size, t_maps_without,
        ...
        %% Other functions
	t_pdict
	%% , t_ets %% Broken for some reason
	%% , t_tracing %% Not available
    ].
...
t_guard_receive(Config) when is_list(Config) ->
    M0  = #{ id => 0 },
    Pid = epiphany:spawn_link(fun() -> guard_receive_loop() end),
...
t_maps_fold(_Config) ->
    ...
    %% fold
    325 = maps:fold(fun({k,_},{v,V},A) -> V + A end, 0, M),
...
t_maps_without(_Config) ->
    ...
    M0 = maps:from_list([{{k,I},{v,I}}||I<-lists:seq(1,25)]),
    M1 = maps:from_list([{{k,I},{v,I}}||I<-lists:seq(1,25) -- Ki]),
