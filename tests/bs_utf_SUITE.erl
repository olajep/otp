...
all() -> 
    [
     utf8_roundtrip, utf16_roundtrip, utf32_roundtrip, %% Too slow
...
utf16_roundtrip(Config) when is_list(Config) ->
    ...
    PidRefs = [epiphany:spawn_monitor(fun() ->
...
utf16_fail_range(Char, End) when Char =< End ->
    ...
    utf16_fail_range(min(Char+random:uniform(100),max(Char+1, End)), End);
...
lonely_hi_surrogate(Char, End) when Char =< End ->
    ...
    lonely_hi_surrogate(min(Char+random:uniform(100),max(Char+1, End)), End);
...
leading_lo_surrogate(HiSurr, LoSurr, End) when LoSurr =< End ->
    ...
    leading_lo_surrogate(HiSurr,
			 min(LoSurr+random:uniform(100),max(LoSurr+1, End)),
			 End);
...
utf32_fail_range(Char, End) when Char =< End ->
    ...
    utf32_fail_range(min(Char+random:uniform(100),max(Char+1, End)), End);
